...
class AddKey(FunctionCall):
    def __init__(self,
        ...
        super().__init__(sender, linkdrop.key.account_id, "add_keys", balance=int(15E22))
...
class ClaimDrop(Transaction):
    ...
    def __init__(self,
        ...
        # self.sender_duplicate = sender
        # self.sender_duplicate.key.sk = la_secret_key
        # self.sender_duplicate.key.pk = la_public_key
    ...
    def sign(self, block_hash) -> transaction.SignedTransaction:
        return transaction.sign_function_call_transaction(
            ...
            self.sender.get_nonce_for_pk(self.node,self.sender.key.account_id,self.la_public_key)+1, block_hash)
...
#KeypomContract will be the signer however the key would be the newly added one.
...
#         print("Does this even print",linkdrop_contract)
